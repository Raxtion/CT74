...
void __fastcall TfrmMain::Timer2Timer(TObject *Sender)
{
	...
	if (g_pMainThread->m_nPressCalMoveIndex[0] >= 50)
	{
        ...
        */
        ...
        else if (g_Motion.GetActualPos(4) < g_IniFile.m_dNLimitF || abs(g_Motion.GetActualPos(4) > g_IniFile.m_dPLimitF)) g_IniFile.m_nErrorCode = 1010;
        else if (g_Motion.GetActualPos(5) < g_IniFile.m_dNLimitR || abs(g_Motion.GetActualPos(5) > g_IniFile.m_dPLimitR)) g_IniFile.m_nErrorCode = 1010;
        else {if (g_IniFile.m_nErrorCode == 1010) g_IniFile.m_nErrorCode = 0;};
