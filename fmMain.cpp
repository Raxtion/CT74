...
void __fastcall TfrmMain::Timer1Timer(TObject *Sender)
{
	...
	if (g_DIO.ReadDIBit(DI::YAxisSafePosB)) Shape2->Brush->Color = clLime;
        ...
        //---Renew UPH
        editUnitPerHour->Text = FormatFloat("0.000", g_pMainThread->m_dUnitPerHour1+g_pMainThread->m_dUnitPerHour0);
...
void __fastcall TfrmMain::Timer2Timer(TObject *Sender)
{
	...
	if (g_pMainThread->m_nPressCalMoveIndex[0] >= 50)
	{
        ...
        if (g_Motion.GetActualPos(4) < g_IniFile.m_dNLimitF || abs(g_Motion.GetActualPos(4) > g_IniFile.m_dPLimitF)) g_IniFile.m_nErrorCode = 1010;
        if (g_Motion.GetActualPos(5) < g_IniFile.m_dNLimitR || abs(g_Motion.GetActualPos(5) > g_IniFile.m_dPLimitR)) g_IniFile.m_nErrorCode = 1010;
