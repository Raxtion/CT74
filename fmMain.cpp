...
void __fastcall TfrmMain::N2Click(TObject *Sender)
{
	...
	if (OpenDialog1->Execute())
	{
        ...
        GroupBox2->Caption = StringReplace(g_IniFile.m_strLastFileName, "C:\\Product Data\\", "",TReplaceFlags());
...
void __fastcall TfrmMain::Timer1Timer(TObject *Sender)
{
    ...
    if (g_pMainThread->m_bIsAutoMode)
    {
        if (fabs(g_IniFile.m_dLamTemp[0] - g_ModBus.GetPV(1))>g_IniFile.m_dAutoRunTempRange) g_IniFile.m_nErrorCode = 32;
        if (fabs(g_IniFile.m_dLamTemp[1] - g_ModBus.GetPV(2))>g_IniFile.m_dAutoRunTempRange) g_IniFile.m_nErrorCode = 31;
...
void __fastcall TfrmMain::Timer2Timer(TObject *Sender)
{
    ...
    //--- renew paintBox per 1 second (if not Cal. manuel will show some stiky number, should off it)
    /*
	g_pMainThread->m_nPressCalMoveIndex[0]++;
	g_pMainThread->m_nLaserCalMoveIndex[0]++;
	g_pMainThread->m_nPressCalMoveIndex[1]++;
	g_pMainThread->m_nLaserCalMoveIndex[1]++;

	if (g_pMainThread->m_nPressCalMoveIndex[0] >= 50)
	{
		g_pMainThread->m_nPressCalMoveIndex[0] = 0;
		g_pMainThread->m_nLaserCalMoveIndex[0] = 0;
		g_pMainThread->m_nPressCalMoveIndex[1] = 0;
		g_pMainThread->m_nLaserCalMoveIndex[1] = 0;
	}
    */
    ...
    {
        if (g_Motion.GetActualPos(0) < -0.1 || abs(g_Motion.GetActualPos(0) > 292.0)) g_pMainThread->m_listLog.push_back("X 撞軟體極限");
        else if (g_Motion.GetActualPos(1) < -7.0 || abs(g_Motion.GetActualPos(1) > 583.0)) g_pMainThread->m_listLog.push_back("Y 撞軟體極限");
        else if (g_Motion.GetActualPos(2) < -2.9 || abs(g_Motion.GetActualPos(2) > 549.0)) g_pMainThread->m_listLog.push_back("LC 撞軟體極限");
        else if (g_Motion.GetActualPos(4) < g_IniFile.m_dNLimitF || abs(g_Motion.GetActualPos(4) > g_IniFile.m_dPLimitF)) g_pMainThread->m_listLog.push_back("FL 撞軟體極限");
        else if (g_Motion.GetActualPos(5) < g_IniFile.m_dNLimitR || abs(g_Motion.GetActualPos(5) > g_IniFile.m_dPLimitR)) g_pMainThread->m_listLog.push_back("RL 撞軟體極限");
    }
