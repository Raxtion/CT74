...
bool TfrmMain::StartProcess(bool bStart)
{
	if(bStart)
	{
        ...
        g_pMainThread->m_ActionLog.push_back(g_pMainThread->AddTimeString("[Execute]收到CIM回應 START，機台開始啟動..."));
	}
	else
	{
		g_pMainThread->m_bIsStopProcessbyCIM = true;
        g_pMainThread->m_listLog.push_back("收到CIM回應 STOP，機台禁止啟動...");
        g_pMainThread->m_ActionLog.push_back(g_pMainThread->AddTimeString("[Execute]收到CIM回應 STOP，機台禁止啟動..."));
