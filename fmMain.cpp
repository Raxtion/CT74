...
void __fastcall TfrmMain::Timer1Timer(TObject *Sender)
{
	...
	if (g_IniFile.m_bNotLam)
	{
		...
		else Caption = g_IniFile.m_strLastFileName + " ( 不壓合 )";
	}
	else
	{
		Caption = g_IniFile.m_strLastFileName;
...
void __fastcall TfrmMain::N8Click(TObject *Sender)
{
	...
	DDX_Float(bRead, g_IniFile.m_dLamHeight[0], pWndRecord->m_dLamHeight0);
	DDX_Float(bRead, g_IniFile.m_dLamHeight[1], pWndRecord->m_dLamHeight1);
	DDX_Float(bRead, g_IniFile.m_dLamStop[0], pWndRecord->m_dLamStop0);
	DDX_Float(bRead, g_IniFile.m_dLamStop[1], pWndRecord->m_dLamStop1);
	DDX_Float(bRead, g_IniFile.m_dLamGetPos[0], pWndRecord->m_dLamGetPos0);
	DDX_Float(bRead, g_IniFile.m_dLamGetPos[1], pWndRecord->m_dLamGetPos1);
	DDX_Float(bRead, g_IniFile.m_dLamVacHeight[0], pWndRecord->m_dLamVacHeight0);
	DDX_Float(bRead, g_IniFile.m_dLamVacHeight[1], pWndRecord->m_dLamVacHeight1);

	DDX_Float(bRead, g_IniFile.m_dLamSecondHeight[0], pWndRecord->m_dLamSecondHeight0);
	DDX_Float(bRead, g_IniFile.m_dLamSecondHeight[1], pWndRecord->m_dLamSecondHeight1);
	DDX_Float(bRead, g_IniFile.m_dLamSecondTime[0], pWndRecord->m_dLamSecondTime0);
	DDX_Float(bRead, g_IniFile.m_dLamSecondTime[1], pWndRecord->m_dLamSecondTime1);
	DDX_Float(bRead, g_IniFile.m_dLamThirdHeight[0], pWndRecord->m_dLamThirdHeight0);
	DDX_Float(bRead, g_IniFile.m_dLamThirdHeight[1], pWndRecord->m_dLamThirdHeight1);
	DDX_Float(bRead, g_IniFile.m_dLamThirdTime[0], pWndRecord->m_dLamThirdTime0);
	DDX_Float(bRead, g_IniFile.m_dLamThirdTime[1], pWndRecord->m_dLamThirdTime1);

	DDX_Int(bRead, g_IniFile.m_nAutoInterval, pWndRecord->m_nAutoInterval);
	DDX_Float(bRead, g_IniFile.m_dAutoStopRange, pWndRecord->m_dAutoStopRange);
	...
	DDX_Float(bRead, g_IniFile.m_dPressCalRange, pWndRecord->m_dPressCalRange);
	DDX_Float(bRead, g_IniFile.m_dPressCalTime, pWndRecord->m_dPressCalTime);

	DDX_Float(bRead, g_IniFile.m_dAutoRunTempRange, pWndRecord->m_dAutoRunTempRange);
	DDX_Float(bRead, g_IniFile.m_dVacDelayTime, pWndRecord->m_dVacDelayTime);
	...
	while (1)
	{
		if (pWnd->ShowModal() == mrOk)
		{
			...
			else if (pWnd->m_dLamStop0->Text.ToDouble() > 10 || pWnd->m_dLamStop1->Text.ToDouble() > 10)
			{
